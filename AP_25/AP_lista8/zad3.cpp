...
ll count(vector<vector<ll> >& children, ll u, ll p){
  for (ll i=0; i < children[u].size(); i++){
    if (children[u][i] != p) {
      paths[u] += count(children,children[u][i], u);
    }
  }
  return paths[u];
};
...
int main(){
    ...
    while(Q--) {
        ll u,v;
        cin>>u>>v;
        paths[u]++, paths[v]++; paths[LCA(lvl,DP,u,v,LOGN)]--;
        if (DP[LCA(lvl,DP,u,v,LOGN)][0]){
          paths[DP[LCA(lvl,DP,u,v,LOGN)][0]]-- ;};
    }
    count(children, 1, 0);
    for (ll i=1; i<=N; i++) {
      cout<<paths[i]<<" ";
